...
import { ArrowLeft, User, Shield, Bell, Palette, Clock } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const sections = [
    { icon: User, title: "Profile", desc: "Account details", items: [
      ...
      { label: "Display name", value: "Coming soon", editable: false, soon: true },
    ]},
    { icon: Shield, title: "Security", desc: "Authentication", items: [
      { label: "Password", value: "Change password", editable: false, soon: true },
      { label: "Two-factor auth", value: "Not enabled", editable: false, soon: true },
    ]},
    { icon: Palette, title: "Appearance", desc: "Customize", items: [
      { label: "Theme", value: "Dark", editable: false, soon: true },
      { label: "Language", value: "English", editable: false },
    ]},
    { icon: Bell, title: "Notifications", desc: "Alerts", items: [
      { label: "Email alerts", value: "Enabled", editable: false, soon: true },
    ]},
  ];

  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "#080510" }}>
      <AmbientBackground />
      <Sidebar />
                ...
                >
                  ...
                  <div className="space-y-1">
                    {items.map(({ label, value, soon }) => (
                      <div key={label} className="flex items-center justify-between py-2.5 px-3 rounded-xl transition-all"
