...
const FEATURES = [
  { icon: Brain,       title: "Streaming AI",       desc: "Word-by-word responses via Server-Sent Events. Zero latency feel." },
  { icon: FileSearch,  title: "RAG Pipeline",        desc: "Upload PDFs. Ask questions. Get answers grounded in your documents." },
  { icon: Shield,      title: "JWT Auth",            desc: "Secure authentication with bcrypt and role-based access control." },
  { icon: Zap,         title: "Redis Caching",       desc: "Rate limiting and response caching for production performance." },
  { icon: BarChart3,   title: "Observability",       desc: "Prometheus metrics, Grafana dashboards, structured logging." },
  { icon: Layers,      title: "Full Stack",          desc: "FastAPI + PostgreSQL + Next.js. Docker Compose. CI/CD ready." },
];

const STATS = [
  { num: "15+", label: "API Endpoints" },
  { num: "RAG", label: "Document AI" },
  { num: "SSE", label: "Live Streaming" },
  { num: "7",   label: "Passing Tests" },
];
...
function FeatureCard({ icon: Icon, title, desc, index }: { icon: any; title: string; desc: string; index: number }) {
  return (
    ...
    >
      ...
      <p className="text-xs leading-relaxed" style={{ color: "hsl(20 8% 55%)" }}>{desc}</p>
    </motion.div>
  );
}
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen overflow-y-auto overflow-x-hidden" style={{ background: "#080510", height: "100vh" }}>
      <AmbientBackground />
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <Link href="/demo"
            ...
            onMouseLeave={(e: any) => (e.target.style.color = "hsl(20 8% 55%)")}
          >
            Live demo
          </Link>
          <Link href="/register"
      ...
      >
        ...
        <motion.h1
          ...
          className="text-6xl md:text-7xl lg:text-8xl font-bold leading-[1.05] mb-6 tracking-tight max-w-5xl"
        >
        ...
        <motion.div
          ...
          className="flex items-center gap-8 mt-16 pt-12"
          style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}
        >
          {STATS.map(({ num, label }) => (
            <div key={label} className="text-center">
              <div className="text-xl font-bold text-white mb-0.5">{num}</div>
              <div className="text-xs" style={{ color: "hsl(20 8% 45%)" }}>{label}</div>
            </div>
          ))}
        </motion.div>
      </motion.section>
      ...
      <section className="relative z-10 px-6 py-24 max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-sm max-w-md mx-auto" style={{ color: "hsl(20 8% 55%)" }}>
            Every component designed with enterprise-grade engineering and recruiter-impressive quality.
          </p>
        </motion.div>
      ...
      <section className="relative z-10 px-6 py-24">
        ...
        >
          ...
          <p className="text-sm mb-8" style={{ color: "hsl(20 8% 55%)" }}>
            Join now and experience the next generation of AI conversations.
          </p>
          <Link href="/register"
            className="inline-flex items-center gap-2.5 px-8 py-4 rounded-2xl text-sm font-semibold text-white"
            style={{
              background: "linear-gradient(135deg, #7c3aed 0%, #6d28d9 100%)",
      ...
      <footer className="relative z-10 px-6 py-8 text-center" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        <p className="text-xs" style={{ color: "hsl(20 8% 35%)" }}>
          Â© 2026 Lumora AI. Built with FastAPI, Next.js, PostgreSQL, Redis, FAISS.
        </p>
      </footer>
    </div>
  );
