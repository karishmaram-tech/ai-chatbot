...
export default function AnalyticsPage() {
  ...
  const stats = usage ? [
    { label: "Conversations", value: usage.total_conversations, icon: MessageSquare, color: "#a78bfa" },
    { label: "Messages", value: usage.total_messages, icon: TrendingUp, color: "#67e8f9" },
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            ...
            {loading ? (
              <div className="grid grid-cols-2 gap-3">
                {[1,2,3,4].map(i => (
                  <div key={i} className="h-24 rounded-2xl" style={{ background: "rgba(255,255,255,0.03)", animation: "skeletonPulse 1.8s ease-in-out infinite" }} />
                ))}
              </div>
            ) : (
              <>
                  ...
                  <div className="rounded-2xl p-5"
                    ...
                    <div className="space-y-1">
                      {usage.recent_conversations.map((conv: any) => (
                        <div key={conv.id} className="flex items-center justify-between py-2.5 px-3 rounded-xl"
