...
export default function RegisterPage() {
  ...
  function fieldStyle(field: string) {
    return {
      ...
      border: error && field !== "password" ? "1px solid rgba(239,68,68,0.4)" : focused === field ? "1px solid rgba(139,92,246,0.5)" : "1px solid rgba(255,255,255,0.07)",
  ...
  return (
    <div className="min-h-screen flex overflow-hidden" style={{ background: "#080510" }}>
      <AmbientBackground />
      <div className="hidden lg:flex flex-col justify-between w-[48%] p-16 relative z-10"
        style={{ borderRight: "1px solid rgba(255,255,255,0.05)" }}>
        <LumoraLogo size="md" />
        <div>
          <h1 className="text-5xl leading-[1.15] mb-5 font-light text-white"
      ...
      <div className="flex flex-1 items-center justify-center p-8 relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.2 }} className="w-full max-w-sm">
            ...
            <div className="space-y-1.5">
              ...
              {password.length > 0 && (
                <div className="flex items-center gap-2 mt-1.5">
                  {[1,2,3].map((l) => (
                    <div key={l} className="flex-1 h-0.5 rounded-full transition-all"
                      style={{ background: strength >= l ? strengthColors[strength] : "rgba(255,255,255,0.08)" }} />
                  ))}
                  <span className="text-[10px]" style={{ color: strengthColors[strength] || "rgba(255,255,255,0.3)" }}>
